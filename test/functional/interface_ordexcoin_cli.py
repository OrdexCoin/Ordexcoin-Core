...
# The block reward of coinbaseoutput.nValue (50) OXC/block matures after
# COINBASE_MATURITY (100) blocks. Therefore, after mining 101 blocks we expect
# node 0 to have a balance of (BLOCKS - COINBASE_MATURITY) * 50 OXC/block.
...
class TestOrdexCoinCli(OrdexCoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Compare responses from getblockchaininfo RPC and `ordexcoin-cli getblockchaininfo`")
        ...
        self.log.info("Test -getinfo and ordexcoin-cli return all proxies")
        ...
        if self.is_specified_wallet_compiled():
            self.log.info("Test -getinfo and ordexcoin-cli getwalletinfo return expected wallet info")
            ...
            assert_equal(Decimal(cli_get_info['Transaction fee rate (-paytxfee) (OXC/kvB)']), wallet_info['paytxfee'])
            assert_equal(Decimal(cli_get_info['Min tx relay fee rate (OXC/kvB)']), network_info['relayfee'])
            ...
            # Mine a block to confirm; adds a block reward (50 OXC) to the default wallet.
            ...
            # Test ordexcoin-cli -generate with -rpcwallet in multiwallet mode.
