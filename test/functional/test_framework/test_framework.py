...
TMPDIR_PREFIX = "ordexcoin_func_test_"
...
class OrdexCoinTestMetaClass(type):
    """Metaclass for OrdexCoinTestFramework.

    Ensures that any attempt to register a subclass of `OrdexCoinTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'OrdexCoinTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("OrdexCoinTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("OrdexCoinTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class OrdexCoinTestFramework(metaclass=OrdexCoinTestMetaClass):
    """Base class for a ordexcoin test script.

    Individual ordexcoin test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave ordexcoinds and test.* datadir on exit or error")
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop ordexcoinds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use ordexcoin-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_ordexcoind = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "ordexcoind" + config["environment"]["EXEEXT"],
        )
        fname_ordexcoincli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "ordexcoin-cli" + config["environment"]["EXEEXT"],
        )
        fname_ordexcoinutil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "ordexcoin-util" + config["environment"]["EXEEXT"],
        )
        self.options.ordexcoind = os.getenv("ORDEXCOIND", default=fname_ordexcoind)
        self.options.ordexcoincli = os.getenv("ORDEXCOINCLI", default=fname_ordexcoincli)
        self.options.ordexcoinutil = os.getenv("ORDEXCOINUTIL", default=fname_ordexcoinutil)
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: ordexcoinds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("OrdexCoinRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'ordexcoind', self.options.ordexcoind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'ordexcoin-cli', self.options.ordexcoincli) for v in versions]
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a ordexcoind test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple ordexcoind test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as ordexcoind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    ordexcoind=self.options.ordexcoind,
                    ordexcoin_cli=self.options.ordexcoincli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in ordexcoin.conf
    ...
    def skip_if_no_ordexcoind_tracepoints(self):
        """Skip the running test if ordexcoind has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("ordexcoind has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_ordexcoind_zmq(self):
        """Skip the running test if ordexcoind has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("ordexcoind has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if ordexcoin-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("ordexcoin-wallet has not been compiled")

    def skip_if_no_ordexcoin_util(self):
        """Skip the running test if ordexcoin-util has not been compiled."""
        if not self.is_ordexcoin_util_compiled():
            raise SkipTest("ordexcoin-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if ordexcoin-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("ordexcoin-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether ordexcoin-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether ordexcoin-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_ordexcoin_util_compiled(self):
        """Checks whether ordexcoin-util was compiled."""
        return self.config["components"].getboolean("ENABLE_ORDEXCOIN_UTIL")
