...
Below are some notes on how to build OrdexCoin Core for Windows.

The options known to work for building OrdexCoin Core on Windows are:
...
Note that for WSL the OrdexCoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/ordexcoin, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\ordexcoin`, for example:

    make install DESTDIR=/mnt/c/workspace/ordexcoin
