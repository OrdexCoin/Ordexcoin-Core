OrdexCoin Core version 0.18.0 is now available from:

  <https://bitcoincore.org/bin/ordexcoin-core-0.18.0/>
...
`/Applications/OrdexCoin-Qt` (on Mac) or `ordexcoind`/`ordexcoin-qt` (on
Linux).
...
recommended to use OrdexCoin Core on unsupported systems.

OrdexCoin Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, OrdexCoin Core does not yet change appearance when
...
`contrib/init/ordexcoind.service`, it has been changed to use
`/var/lib/ordexcoind` as the data directory instead of
`~ordexcoin/.ordexcoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/ordexcoind` will
exist has enough space (check using `df -h /var/lib/ordexcoind`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/ordexcoin/ordexcoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:25173:25173` (this is an extra `:25173` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://bitcoincore.org/en/contact/) the OrdexCoin Core
...
The systemd init file (`contrib/init/ordexcoind.service`) has been changed
to use `/var/lib/ordexcoind` as the data directory instead of
`~ordexcoin/.ordexcoin`. This change makes OrdexCoin Core more consistent with
...
`datadir` option in `/etc/ordexcoin/ordexcoin.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/ordexcoin/ordexcoin.conf`.


Documentation
-------------

- A new short [document](https://github.com/ordexcoin/ordexcoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/ordexcoin/ordexcoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/ordexcoin/ordexcoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/ordexcoin/ordexcoin/blob/master/doc/ordexcoin-conf.md)
  about the `ordexcoin.conf` file describes how to use it to configure
  OrdexCoin Core.

- A new document introduces OrdexCoin Core's BIP174 [Partially-Signed
  OrdexCoin Transactions
  (PSBT)](https://github.com/ordexcoin/ordexcoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/ordexcoin/ordexcoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent OrdexCoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  OrdexCoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/ordexcoin/ordexcoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/ordexcoin/ordexcoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when OrdexCoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of OrdexCoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/ordexcoin/ordexcoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/ordexcoin/ordexcoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart ordexcoind with the `-deprecatedrpc=generate`
...
- A new `ordexcoin-wallet` tool is now distributed alongside OrdexCoin
...
This section describes planned changes to OrdexCoin Core that may affect
other OrdexCoin software and services.

- Since version 0.16.0, OrdexCoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with OrdexCoin Core 0.20 (expected about
  a year after 0.18), OrdexCoin Core will default to native segwit
  ...
  to bech32 addresses, and if the OrdexCoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in OrdexCoin Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any OrdexCoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, OrdexCoin Core
...
- A sub-project of OrdexCoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with OrdexCoin Core.  See their [project
  page](https://github.com/ordexcoin-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to OrdexCoin Core's own implementation, although entropy
  gathered by OrdexCoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves OrdexCoin Core a
...
- On macOS, OrdexCoin Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `ordexcoin_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libordexcoin_server.a` (jnewbery)
...
- #13926 [Tools] ordexcoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like ordexcoind (MarcoFalke)
...
- #14521 qt, docs: Fix `ordexcoin-qt -version` output formatting (hebasto)
...
- #11625 Add OrdexCoinApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libordexcoin_server` (laanwj)
...
- #15154 Configure: ordexcoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/ordexcoin-wallet (Sjors)
...
- #11526 Visual Studio build configuration for OrdexCoin Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libordexcoin_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in ordexcoin-util-test.py (isghe)
...
- #12775 Integration of property based testing into OrdexCoin Core (Christewart)
...
- #12246 Bugfix: Only run ordexcoin-tx tests when ordexcoin-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_ordexcoin` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_ordexcoin` (MarcoFalke)
...
- #14186 ordexcoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_ordexcoin.conf` message on startup if conf file exists (leishman)
...
- #12255 Update ordexcoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/ordexcoin-conf.md (hebasto)
...
- #15354 Add missing `ordexcoin-wallet` tool manpages (MarcoFalke)
...
- #15765 correct ordexcoinconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support ordexcoin folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/ordexcoin/).
