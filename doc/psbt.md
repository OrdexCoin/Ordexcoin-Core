# PSBT Howto for OrdexCoin Core

Since OrdexCoin Core 0.17, an RPC interface exists for Partially Signed OrdexCoin
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/ordexcoin/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for OrdexCoin transactions that are not fully signed
...
#### Multisig with multiple OrdexCoin Core instances
...
OrdexCoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `ordexcoin-cli` is used.
...
Later, when *V* OXC has been received on *Amulti*, and Bob and Carol want to
