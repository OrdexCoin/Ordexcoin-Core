# User-space, Statically Defined Tracing (USDT) for OrdexCoin Core

OrdexCoin Core includes statically defined tracepoints to allow for more
...
**Note**: OrdexCoin Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to OrdexCoin Core
...
Multiple tools can list the available tracepoints in a `ordexcoind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in OrdexCoin Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/ordexcoind
stap net        outbound_message 0x0000000000107c05 /src/ordexcoind
stap validation block_connected  0x00000000002fb10c /src/ordexcoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in OrdexCoin Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/ordexcoind | grep NT_STAPSDT -A 4 -B 2
