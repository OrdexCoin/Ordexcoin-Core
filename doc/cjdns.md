# CJDNS support in OrdexCoin Core

It is possible to run OrdexCoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of OrdexCoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the OrdexCoin network and individual nodes.
...
before it will work with your OrdexCoin Core node. This is described in steps
...
## Run OrdexCoin Core with CJDNS

Once you are connected to the CJDNS network, the following OrdexCoin Core
...
When enabled, this option tells OrdexCoin Core that it is running in an
...
IPv6 local network. This helps OrdexCoin Core perform better address management:
...
CJDNS support was added to OrdexCoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `ordexcoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/ordexcoin/ordexcoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in OrdexCoin Core

There are several ways to see your CJDNS address in OrdexCoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `ordexcoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `ordexcoin-cli getpeerinfo`).
