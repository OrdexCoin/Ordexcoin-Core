...
def parse_config_into_oxc_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"OrdexCoin Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/ordexcoin_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/ordexcoin_config.h'), "w", encoding="utf8") as oxc_config:
        oxc_config.writelines(template)
...
def main():
    parser = argparse.ArgumentParser(description='OrdexCoin-core msbuild configuration initialiser.')
    ...
    parse_config_into_oxc_config()
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/ordexcoin_config.h'), os.path.join(SOURCE_DIR, 'config/ordexcoin-config.h'))
