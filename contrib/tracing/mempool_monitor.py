...
""" Example logging OrdexCoin Core mempool events using the mempool:added,
...
def main(ordexcoind_path):
    ordexcoind_with_usdts = USDT(path=str(ordexcoind_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    ordexcoind_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    ordexcoind_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    ordexcoind_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    ordexcoind_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[ordexcoind_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/ordexcoind")
        exit(1)
