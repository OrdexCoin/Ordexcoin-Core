...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing ordexcoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='ordexcoin-cli', help='ordexcoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='OrdexCoin address to which the faucet should send')
...
parser.add_argument('ordexcoin_cli_args', nargs='*', help='Arguments to pass on to ordexcoin-cli (default: -signet)')

args = parser.parse_args()

if args.ordexcoin_cli_args == []:
    args.ordexcoin_cli_args = ['-signet']


def ordexcoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.ordexcoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = ordexcoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = ordexcoin_cli(['getnewaddress', 'faucet', 'bech32'])
