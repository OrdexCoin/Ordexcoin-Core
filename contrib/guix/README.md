# Bootstrappable OrdexCoin Core Builds

This directory contains the files necessary to perform bootstrappable OrdexCoin
Core builds.
...
To build OrdexCoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      OrdexCoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/ordexcoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/ordexcoin-core/guix.sigs).
