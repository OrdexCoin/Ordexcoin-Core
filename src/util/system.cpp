...
const char * const ORDEXCOIN_CONF_FILENAME = "ordexcoin.conf";
const char * const ORDEXCOIN_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //ordexcoin-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", ORDEXCOIN_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\OrdexCoin
    // macOS: ~/Library/Application Support/OrdexCoin
    // Unix-like: ~/.ordexcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "OrdexCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/OrdexCoin";
#else
    // Unix-like
    return pathRet / ".ordexcoin";
#endif
#endif
}
...
{
    return GetConfigFile(*this, GetPathArg("-conf", ORDEXCOIN_CONF_FILENAME));
}
