...
#include <qt/ordexcoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a OrdexCoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new OrdexCoinAddressEntryValidator(parent));
    widget->setCheckValidator(new OrdexCoinAddressCheckValidator(parent));
}
...
bool parseOrdexCoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no ordexcoin: URI
    if(!uri.isValid() || uri.scheme() != QString("ordexcoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!OrdexCoinUnits::parse(OrdexCoinUnit::OXC, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseOrdexCoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseOrdexCoinURI(uriInstance, out);
}

QString formatOrdexCoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("ordexcoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(OrdexCoinUnits::format(OrdexCoinUnit::OXC, info.amount, false, OrdexCoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openOrdexCoinConf()
{
    ...
    /* Open ordexcoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "OrdexCoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "OrdexCoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("OrdexCoin (%s).lnk", chain));
}

bool GetStartOnSystemStartup()
{
    // check for OrdexCoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "ordexcoin.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("ordexcoin-%s.desktop", chain));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a ordexcoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=OrdexCoin\n";
        else
            optionFile << strprintf("Name=OrdexCoin (%s)\n", chain);
