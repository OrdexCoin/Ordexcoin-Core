...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<OrdexCoinUnit>("OrdexCoinUnit");
#else
    qRegisterMetaType<OrdexCoinUnit>("OrdexCoinUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. ordexcoin_de.qm (shortcut "de" needs to be defined in ordexcoin.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. ordexcoin_de_DE.qm (shortcut "de_DE" needs to be defined in ordexcoin.qrc)
...
static const char* qt_argv = "ordexcoin-qt";

OrdexCoinApplication::OrdexCoinApplication()
...
void OrdexCoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the OrdexCoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", OrdexCoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void OrdexCoinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool OrdexCoinApplication::createOptionsModel(bool resetSettings)
{
...
void OrdexCoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new OrdexCoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &OrdexCoinGUI::quitRequested, this, &OrdexCoinApplication::requestShutdown);
...
void OrdexCoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void OrdexCoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool OrdexCoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void OrdexCoinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &OrdexCoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &OrdexCoinApplication::handleRunawayException);
    connect(this, &OrdexCoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &OrdexCoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void OrdexCoinApplication::parameterSetup()
{
...
void OrdexCoinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void OrdexCoinApplication::requestInitialize()
{
...
void OrdexCoinApplication::requestShutdown()
{
...
void OrdexCoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success) {
        ...
        // ordexcoin: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &OrdexCoinGUI::handlePaymentRequest);
            connect(window, &OrdexCoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void OrdexCoinApplication::handleRunawayException(const QString &message)
{
...
void OrdexCoinApplication::handleNonFatalException(const QString& message)
{
...
WId OrdexCoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool OrdexCoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", OrdexCoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(ordexcoin);
    Q_INIT_RESOURCE(ordexcoin_locale);
    ...
    /// 6-7. Parse ordexcoin.conf, determine network, switch to network specific
    ...
    // ordexcoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
