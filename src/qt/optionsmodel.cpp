...
/** Call node.updateRwSetting() with OrdexCoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const util::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because OrdexCoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/ordexcoin/ordexcoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because ordexcoin 22.x will not try to read these.
...
//! Convert enabled/size values to ordexcoin -prune setting.
...
//! Get pruning enabled value to show in GUI from ordexcoin -prune setting.
...
//! Get pruning size value to show in GUI from ordexcoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayOrdexCoinUnit")) {
        settings.setValue("DisplayOrdexCoinUnit", QVariant::fromValue(OrdexCoinUnit::OXC));
    }
    QVariant unit = settings.value("DisplayOrdexCoinUnit");
    if (unit.canConvert<OrdexCoinUnit>()) {
        m_display_ordexcoin_unit = unit.value<OrdexCoinUnit>();
    } else {
        m_display_ordexcoin_unit = OrdexCoinUnit::OXC;
        settings.setValue("DisplayOrdexCoinUnit", QVariant::fromValue(m_display_ordexcoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if ordexcoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_ordexcoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<OrdexCoinUnit>() == m_display_ordexcoin_unit) return;
    m_display_ordexcoin_unit = new_unit.value<OrdexCoinUnit>();
    QSettings settings;
    settings.setValue("DisplayOrdexCoinUnit", QVariant::fromValue(m_display_ordexcoin_unit));
    Q_EMIT displayUnitChanged(m_display_ordexcoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/ordexcoin/ordexcoin/pull/8273
    ...
    // (https://github.com/ordexcoin-core/gui/issues/567).
